...
- **Trace a binary (copies all files accessed during run):**
    ...
    - **Exact launch form.** `--trace` performs a single sandboxed `execv()` of `/usr/bin/strace` with the fixed token sequence `/usr/bin/strace -f -e trace=file -o /tmp/straceXXXXXX <trace_target> <args...>` (`sandbox.c:876-886`). The `-o` path is the exact template passed to `mkstemp("/tmp/straceXXXXXX")` at `sandbox.c:858-864` — `mkstemp()` overwrites the six `X`s in place with a unique suffix, so every run gets a fresh `/tmp/strace<6chars>` file. `<trace_target>` is selected at `sandbox.c:872-875`: the original absolute `target` string when `target[0] == '/'`, otherwise `/usr/bin/<basename>` (resolved from `target_name`, which `build_rootfs()` sets to `basename(bin)` at `sandbox.c:664`). `<args...>` is every argv token after `--trace`, copied verbatim in order (`sandbox.c:884-885`). `trace_main()` does nothing except call `sandbox_exec(trace_argv)` (`sandbox.c:595-600`), which `execv()`s that argv directly with no wrapper shell.
    - **Exit-status propagation.** After the cloned trace child exits, `main()` `waitpid()`s it and returns the traced child's status to the shell: `WEXITSTATUS(status)` on normal exit, or `128 + WTERMSIG(status)` on signal termination (`sandbox.c:893-898`, `sandbox.c:925-928`). The `[sandbox --trace exited with N]` / `[sandbox --trace killed by signal N]` message is printed before the trace-output scrape but reports the same status that is subsequently returned. Concretely, `sudo ./sandbox /tmp/sb /bin/true --trace; echo $?` prints `0`, and `sudo ./sandbox /tmp/sb /bin/false --trace; echo $?` prints `1`.
