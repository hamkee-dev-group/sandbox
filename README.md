...
- **Add extra files:**
    ...
    ```
    - `extras.txt` contains one path per line. `copy_extras()` reads the file line-by-line and **only copies lines whose first byte is `/`** (`sandbox.c:322-326`); blank lines and relative/non-absolute paths are silently skipped with no warning or error, so only a subset of the listed entries may end up in the rootfs. For each absolute entry, `copy_extras()` calls `copy_file(path, <rootfs><path>)` and, **if the copy fails** (missing source, permission error, etc.), prints `[WARN] Failed to copy extra: <path>` to stderr and **continues** to the next entry (`sandbox.c:329-332`). The run only aborts if `copy_extras()` itself returns `< 0`, which happens only when `fopen(listfile)` fails (`sandbox.c:316-320`, checked at `sandbox.c:807-809` and `sandbox.c:852-854` where `main()` prints `Failed to copy extras` and exits). In other words: a missing or unreadable **list file** is fatal, but individual list **entries** that are blank, relative, or whose source cannot be copied are not — they produce at most a `[WARN]` and the run proceeds.
