...
- **Run a specific binary:**
    ...
    - After those copies, `build_rootfs()` calls `create_dev_nodes(rootfs)` and `create_etc_files(rootfs)` (`sandbox.c:710`). That always ensures `<rootfs>/dev` and `<rootfs>/etc` exist. Outside `--userns`, `create_dev_nodes()` creates `/dev/null`, `/dev/zero`, and `/dev/tty` as character device nodes; in `--userns`, it creates placeholder files that `setup_sandbox_environment()` later bind-mounts over with the host devices. `create_etc_files()` writes `/etc/passwd` and `/etc/group` only when `--user` is enabled, and `--extras` may add other files under `/etc` regardless of mode.
...
- `create_etc_files()` always ensures `/etc` exists, but writes `/etc/passwd` and `/etc/group` only when `--user` enables `drop_to_nobody`
