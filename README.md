...
**sandbox** is a minimalist, auditable, and hackable C program that builds a chrooted Linux environment around a target ELF binary or a minimal shell environment, isolating execution in dedicated namespaces with tight controls on filesystem, user privileges, and process capabilities. The target is validated by `is_binary()` (`sandbox.c:211-260`), which checks that the path is executable (`access(X_OK)`), is a regular file (`S_ISREG`), starts with the ELF magic `\x7fELF`, has `EI_CLASS` set to `ELFCLASS32` or `ELFCLASS64`, has `EI_DATA` set to `ELFDATA2LSB` or `ELFDATA2MSB`, has `EI_VERSION == EV_CURRENT`, has `e_type` set to `ET_EXEC` or `ET_DYN`, and has `e_machine != EM_NONE`. Shell scripts and other non-ELF executables are rejected at this initial check with `"<path> is not a binary file"`; malformed or truncated ELF headers are rejected by `validate_target_binary()` (`sandbox.c:262-273`) with `"<path> is not a valid ELF executable"`.
...
- **Run a specific binary:**
    ...
    - `<target-binary>` must be a regular ELF executable, not a script or a symlink to a non-regular file. It is validated by `is_binary()` (`sandbox.c:211-260`), which checks that the path is executable (`access(X_OK)`), is a regular file (`S_ISREG`), starts with the ELF magic `\x7fELF`, has `EI_CLASS` set to `ELFCLASS32` or `ELFCLASS64`, has `EI_DATA` set to `ELFDATA2LSB` or `ELFDATA2MSB`, has `EI_VERSION == EV_CURRENT`, has `e_type` set to `ET_EXEC` or `ET_DYN`, and has `e_machine != EM_NONE`. This is still lightweight header validation, not full ELF validation: shell scripts and other non-ELF executables are rejected here with `"<path> is not a binary file"` (for example, `./sandbox /tmp/sb-review /tmp/sb-script --userns` on an executable shell script fails immediately with `/tmp/sb-script is not a binary file`), while malformed or truncated ELF headers, including an executable regular file containing only the four ELF magic bytes, are rejected by `validate_target_binary()` (`sandbox.c:262-273`) with `"<path> is not a valid ELF executable"`.
...
- **Trace a binary (replays a filtered subset of strace-reported paths into the rootfs):**
    ...
    - **Exact launch form.** `--trace` performs a single sandboxed `execv()` of `/usr/bin/strace` with the fixed token sequence `/usr/bin/strace -f -e trace=file -o /tmp/strace<6chars> <trace_target> <args-after---trace>` (`sandbox.c:1517-1527`). `main()` first rejects an oversized trace argv, then creates the log with `mkstemp()` using the host-visible template `<rootfs>/tmp/straceXXXXXX`; the generated chroot-relative `/tmp/strace<6chars>` path is what gets passed to strace (`sandbox.c:1491-1516`). Because `strace` is only exec'd after `setup_sandbox_environment()` calls `chroot(rootfs)` (`sandbox.c:453`), that `-o` path names the same rootfs file the parent later reopens at `<rootfs>/tmp/strace<6chars>`. No trace tempfile is created in the host `/tmp`. `<trace_target>` is selected at `sandbox.c:1496-1501`: the original absolute `target` string when `target[0] == '/'`, otherwise `/usr/bin/<basename>` (resolved from `target_name`, which `build_rootfs()` sets to `basename(bin)` at `sandbox.c:664`). `<args-after---trace>` is every argv token after `--trace`, copied verbatim in order (`sandbox.c:1525-1526`). `trace_main()` does nothing except call `sandbox_exec(trace_argv)` (`sandbox.c:595-600`), which `execv()`s that argv directly with no wrapper shell.
    ...
    - **Post-trace file replay.** After the traced child exits, `sandbox` reopens the trace log at `<rootfs>/tmp/strace<6chars>` and scans it line-by-line (`sandbox.c:1542-1570`). The replay is narrower than "every file accessed during the run": lines whose syscall result tail parses as `= -<digits>` are skipped, so failed accesses such as `= -1 ENOENT` are not replayed; lines without a parseable result tail keep the previous best-effort behavior. For each remaining line it locates the **first** pair of double quotes and treats the enclosed string as a candidate path; the candidate is then copied into the rootfs **only** if (1) it is quoted in the trace line, (2) it starts with `/` (absolute), and (3) it does not contain the substring `/..`. Paths that pass those filters are copied via `copy_file(path, <rootfs><path>)`, whose **return value is ignored** — any failure during this replay pass (missing source, permission error, destination write error) is silently skipped and does not abort the run. After the scan completes, `sandbox` **attempts** to `unlink()` the trace log at `<rootfs>/tmp/strace<6chars>` (`sandbox.c:1570`); the `unlink()` return value is also ignored, so on the common success path the file is removed, but a failed `unlink()` does not abort the run and can leave the trace file in place. A post-run check like `test ! -e <rootfs>/tmp/strace*` therefore confirms the expected success path but is not guaranteed by the code.
...
- **Add extra files:**
    ...
    - Copied files preserve the source descriptor's permission bits (`mode & 0777`); setuid, setgid, and sticky bits are not copied. For example, a mode 0600 configuration remains mode 0600 in the rootfs.
...
- Optionally traces binary with `strace` to discover runtime file dependencies. `sandbox` creates the trace log under `<rootfs>/tmp/strace<6chars>` and passes the chroot-relative `/tmp/strace<6chars>` path to strace, so no trace tempfile is created in the host `/tmp`. After the traced child exits, `sandbox` parses that rootfs trace log, skips lines whose syscall result tail is negative, copies into the rootfs only quoted absolute paths that do not contain `/..`, ignores any `copy_file()` failure during this replay pass, and then attempts to `unlink()` the trace log (the `unlink()` return value is also ignored, so removal is best-effort) (`sandbox.c:1503-1570`)
...
- **Namespaces**: all three `clone()` sites — shell mode at `sandbox.c:1436`, `--trace` at `sandbox.c:1522`, and the normal target path at `sandbox.c:1574` — use `CLONE_NEWUTS | CLONE_NEWPID | CLONE_NEWNS | SIGCHLD`, so the sandbox always enters new **UTS** (hostname), **PID** (process IDs), and **mount** (filesystem) namespaces. Root-mode shell and normal target runs additionally OR in `CLONE_NEWNET`; `--trace` always includes `CLONE_NEWNET`, which gives traced programs a separate network namespace even though trace mode skips seccomp. A **user** namespace is added only under `--userns`, which OR's in `CLONE_NEWUSER` for shell mode and normal target mode; `--trace` never adds `CLONE_NEWUSER` because `--trace` and `--userns` are mutually exclusive. IPC and cgroup namespaces are **not** created — see Limitations & Roadmap
...
- **IPC and cgroup namespaces are not created** — the clone-flag sets do not include `CLONE_NEWIPC` or `CLONE_NEWCGROUP`, so sandboxed processes share System V / POSIX IPC objects, POSIX message queues, and the cgroup hierarchy with the host. Root-mode shell, `--trace`, and root-mode normal target runs do include `CLONE_NEWNET`; `--userns` shell and normal target runs do not currently add a network namespace.
